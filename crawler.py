...
_DEADLINE_PATTERNS = [
    ...
    # 2026.07.03.(금)까지 / 7.3.까지 / 7.3. 마감
    re.compile(
        r"(?:(?P<y>\d{4})\s*[.\-/]\s*)?(?P<m>\d{1,2})\s*[.\-/]\s*(?P<d>\d{1,2})\s*\.?\s*(?:\([^)]*\)\s*)?(?:\d{1,2}:\d{2}\s*)?(?:까지|마감)"
    ),
    # 10월 12일 마감 / 7월 3일(금) 마감
    re.compile(r"(?P<m>\d{1,2})월\s*(?P<d>\d{1,2})일\s*(?:\([^)]*\)\s*)?(?:\d{1,2}:\d{2}\s*)?마감"),
]
